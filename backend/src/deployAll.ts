  ...
  { name: "CakeFactory", 
    ...
    postDeploy: async (web3: Web3, addr: Record<string, string>, acct: any, out: any) => {
      ...
      try {
        ...
        const gas = await (registry.methods as any).grantRole(bakerRole, factoryAddr).estimateGas({ from: acct.address });

        await (registry.methods as any).grantRole(bakerRole, factoryAddr).send({ from: acct.address, gas });
...
main().catch(e => { console.error(e); process.exit(1); });      const registryAbi = out.contracts[registryKey]["CakeLifecycleRegistry"].abi;
      const registry = new web3.eth.Contract(registryAbi as any, registryAddr);

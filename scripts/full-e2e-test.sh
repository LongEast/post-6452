...
# 
# Description:
#   This script provides a complete end-to-end test that can be run by anyone
#   who clones the project. It handles all setup automatically:
...
if test_endpoint "POST" "/api/factory/quality-check" "$quality_data" 200 "Factory Quality Check"; then
    PASSED_TESTS=$((PASSED_TESTS + 1))
fi
echo ""
...
# Test 9: Handoff to Shipper (using dynamic address)
handoff_data="{
  \"batchId\": $BATCH_ID,
  \"shipperAddress\": \"$SHIPPER_ADDRESS\"
}"
if test_endpoint "POST" "/api/factory/handoff" "$handoff_data" 200 "Handoff to Shipper"; then
    PASSED_TESTS=$((PASSED_TESTS + 1))
fi
echo ""
...
# Test 11: Deliver to Warehouse (using dynamic address)
deliver_data="{
  \"batchId\": $BATCH_ID,
  \"warehouseAddress\": \"$WAREHOUSE_ADDRESS\"
}"
if test_endpoint "POST" "/api/shipper/deliver" "$deliver_data" 200 "Deliver to Warehouse"; then
    PASSED_TESTS=$((PASSED_TESTS + 1))
fi
echo ""
...
if test_endpoint "POST" "/api/warehouse/confirm-delivery" "$confirm_data" 200 "Confirm Warehouse Delivery"; then
    PASSED_TESTS=$((PASSED_TESTS + 1))
fi
echo ""

# Test 13: Get Updated Batch Record
if test_endpoint "GET" "/api/lifecycle/batch/$BATCH_ID" "" 200 "Get Updated Batch Record"; then
    PASSED_TESTS=$((PASSED_TESTS + 1))
fi
echo ""

# Test 14: Get Comprehensive Audit Data
if test_endpoint "GET" "/api/auditor/batch/$BATCH_ID/comprehensive" "" 200 "Get Comprehensive Audit Data"; then
    PASSED_TESTS=$((PASSED_TESTS + 1))
fi
echo ""

# Test 15: Get Batch Alerts
if test_endpoint "GET" "/api/oracle/batch/$BATCH_ID/alerts" "" 200 "Get Batch Alerts"; then
    PASSED_TESTS=$((PASSED_TESTS + 1))
fi
echo ""
...
if [ $PASSED_TESTS -eq $TOTAL_TESTS ]; then
    ...
    echo ""
    echo -e "${YELLOW}To view the complete lifecycle:${NC}"
    echo "curl $API_BASE/api/auditor/batch/$BATCH_ID/comprehensive | jq ."
    EXIT_CODE=0
else
...
echo -e "  - ganache.log (Ganache blockchain output)"
echo -e "  - api.log (API server output)"

echo ""
echo -e "${YELLOW}Processes will be cleaned up automatically on script exit${NC}"
